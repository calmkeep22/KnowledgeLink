...
응답의 `matches`는 유사도 순 과거 업무 상위 K개(`score`는 코사인 유사도)다. `explanation`의 `similarWork`·`suggestedApproach` 문장은 `evidenceIds`로 `matches`의 과거 업무만 가리키며, 서버가 이를 검증한다. `experiencedMembers`는 1위 유사도의 절반 이상인 결과의 담당자를 유사도 합으로 묶은 것으로, 이번 질의와 관련된 경험의 근거일 뿐 평가나 순위가 아니다.
...
| `KL_DEMO_SIMILAR_MAX_PER_MINUTE` | 20 | 캐시되지 않은 새 질의가 공급자를 부를 수 있는 분당 횟수. 넘으면 503 |
| `KL_DEMO_SIMILAR_CACHE_SIZE` | 200 | 결과를 보관할 서로 다른 질의 수(LRU) |

같은 질의는 공백 차이를 무시하고 캐시에서 바로 응답한다. 과거 업무 색인은 기동 직후 만들고, 실패하면 첫 검색에서 다시 시도한다. 예시 질문 네 개를 심사 전에 한 번씩 눌러 두면 이후에는 공급자를 호출하지 않는다.
