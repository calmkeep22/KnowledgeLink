...
기술 스택은 Java 21·Spring Boot 4.1, React·TypeScript(예정), PostgreSQL·pgvector·pg_trgm이다. 관측은 OpenTelemetry·Prometheus·Grafana로 한다([ADR 0005](docs/adr/0005-opentelemetry-otlp-observability.md)). 앱은 AWS 서버 1대에 Docker Compose·Nginx로, DB는 Amazon RDS for PostgreSQL로 배포하며([ADR 0004](docs/adr/0004-rds-postgresql-for-production.md)) 개발은 로컬과 fake AI로 시작한다. 실제 AI 공급자·모델과 서버 상품은 평가 전에 결정한다.
...
관측 스택은 선택이다. 메모리를 더 쓰므로 필요할 때만 켠다. `otel` 프로필을 켜면 지표·트레이스를 OTLP로 내보내고, Grafana(http://localhost:3000)에서 Prometheus 지표와 Tempo 트레이스를 본다. 작업 엔진 지표는 `kl.jobs`로 시작한다.

```bash
docker compose --profile observability up -d
./gradlew bootRun --args="--spring.profiles.active=local,otel"
```
